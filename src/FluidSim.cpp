...
bool FluidSim::advance() {
	...
	for (int iter = 0; iter < solverIterations; ++iter) {
		...
		for (int i = 0; i < n; ++i) {
			...
			for (int j = 0; j < n; ++j) {
				//delta_pi += (lambdas[i] + lambdas[j]) * gradSpikyKernel(renderPositions->row(i) - renderPositions->row(j), NEIGHBOURHOOD_RADIUS);
				const double k = 0.1;
				const Eigen::Vector3f delta_q = Eigen::Vector3f(0.1, 0.1, 0.1) / 3 * NEIGHBOURHOOD_RADIUS;
				float s_corr = -k * std::pow(poly6Kernel(positions->row(i) - positions->row(j), NEIGHBOURHOOD_RADIUS) / poly6Kernel(delta_q, NEIGHBOURHOOD_RADIUS), 4.);
				delta_pi += (lambdas[i] + lambdas[j] + s_corr) * gradSpikyKernel(positions->row(i) - positions->row(j), NEIGHBOURHOOD_RADIUS);
			}
